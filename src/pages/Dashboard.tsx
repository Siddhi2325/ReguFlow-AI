...
import {
  ...
  Globe, ExternalLink, ShieldAlert, Loader2
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const { dateRange, isWithinRange } = useDateFilter();
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const { signed_url } = await api.generateReport("executive", fromStr, toStr);
      if (signed_url) {
        ...
        link.href = blobUrl;
        link.download = `executive-report-${dateRange.label.replace(/[^a-zA-Z0-9]/g, '_')}-${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  const filteredMaps = useMemo(() => {
    ...
    let result = maps;
    if (selectedDepts.length > 0) result = result.filter((m: any) => selectedDepts.includes(m.department));
    // Apply date filter on dueDate
    result = result.filter((m: any) => isWithinRange(m.dueDate));
    return result;
  }, [maps, orgProfile.departments, isWithinRange]);
  ...
  const filteredRecentActivity = useMemo(() => {
    ...
    let result = enabled.length === 0 ? list : list.filter((r: any) => enabled.includes(r.source));
    // Apply date filter
    result = result.filter((r: any) => isWithinRange(r.date || r.created_at));
    return result;
  }, [data?.recentActivity, orgProfile.enabledSources, isWithinRange]);
  ...
  interface CardMeta2 extends CardMeta { navPath?: string; }
  const kpiCards: CardMeta2[] = [
    {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-border">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={exporting}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 transition-colors uppercase tracking-wider disabled:opacity-50"
          >
            {exporting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <TrendingUp style={{ width: 14, height: 14 }} />}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {kpiCards.map((card: any, i) => (
          <MagneticCard
            key={card.label}
            glowColor={card.glowColor}
            className="flex flex-col justify-between"
            onClick={card.navPath ? () => nav(card.navPath) : undefined}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <MagneticCard className="flex flex-col justify-between" onClick={() => nav("/department-routing")}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Click to view department routing →</p>
            </div>
          </div>
