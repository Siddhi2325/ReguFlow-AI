...
export default function Reports() {
  ...
  // Pre-set date filter if coming from Dashboard "Export Report" button
  useEffect(() => {
    const from = searchParams.get("from");
    const to = searchParams.get("to");
    if (from && to) {
      setCustomRange(new Date(from), new Date(to));
    }
  }, []);
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const { signed_url } = await api.generateReport(active, fromStr, toStr);
      if (signed_url) {
        ...
        link.href = blobUrl;
        link.download = `${active}-report-${dateRange.label.replace(/[^a-zA-Z0-9]/g, '_')}-${new Date().toISOString().slice(0, 10)}.pdf`;
